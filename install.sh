...
#    REMOTE_CONFIG=true
#else
    printf "       Host name [resiotgateway]:"
    read NEW_HOSTNAME
    if [[ $NEW_HOSTNAME == "" ]]; then NEW_HOSTNAME="resiotgateway"; fi

    printf "       Descriptive name [rpic880a]:"
    read GATEWAY_NAME
    if [[ $GATEWAY_NAME == "" ]]; then GATEWAY_NAME="rpic880a"; fi
...
if [ ! -d lora_gateway ]; then
    git clone https://github.com/resiot/lora_gateway.git
    pushd lora_gateway
else
    pushd lora_gateway
    git reset --hard
...
#    popd
	...
	echo "Please insert Host Address UDP Server (EXAMPLE:eu72udp.resiot.io)"
	read varudpserver
	
	echo "Please insert UDP Port (EXAMPLE:7677)"
	read port

    echo -e "{\n\t\"gateway_conf\": {\n\t\t\"gateway_ID\": \"$GATEWAY_EUI\",\n\t\t\"servers\": [ { \"server_address\": \"$varudpserver\", \"serv_port_up\": $port, \"serv_port_down\": $port, \"serv_enabled\": true } ],\n\t\t\"ref_latitude\": $GATEWAY_LAT,\n\t\t\"ref_longitude\": $GATEWAY_LON,\n\t\t\"ref_altitude\": $GATEWAY_ALT,\n\t\t\"contact_email\": \"$GATEWAY_EMAIL\",\n\t\t\"description\": \"$GATEWAY_NAME\" \n\t}\n}" >$LOCAL_CONFIG_FILE
#fi
...
cp ./resiotgateway.service /lib/systemd/system/
systemctl enable resiotgateway.service

echo "The system will reboot in 7 seconds..."
sleep 7
shutdown -r now
